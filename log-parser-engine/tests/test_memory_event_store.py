...
from log_parser_engine.storage.options import EventStoreOptions, EventWriteOptions
...
def test_duplicate_policy_replace(make_log_event):
    ...
    store = InMemoryEventStore(EventStoreOptions(duplicate_policy="replace", identity_mode="content_hash"))
    event1 = make_log_event("message", timestamp=datetime.now(timezone.utc) - timedelta(minutes=1))
    
    res1 = store.add(event1)
    assert res1.status == "inserted"
    assert store.count() == 1
    
    time.sleep(0.01) # ensure inserted_at is different
    event2 = make_log_event("message", attributes={"new": "attr"}) # Same content hash basis
    
    res2 = store.add(event2)
    assert res2.status == "replaced"
    assert store.count() == 1
    
    stored = store.get(res1.stored_event.id)
    assert stored.id == res1.stored_event.id
    assert stored.sequence == res1.stored_event.sequence # Sequence should be preserved
    assert stored.event.attributes == {"new": "attr"}
...
def test_id_collision_error(store: InMemoryEventStore):
    ...
    # Force an ID collision by manipulating the event_id factory
    event1 = LogEvent(event_id=uuid4(), timestamp=datetime.now(timezone.utc), source_type=LogSourceType.FILE, message="msg1", raw_message="msg1")
    event2 = LogEvent(event_id=event1.event_id, timestamp=datetime.now(timezone.utc), source_type=LogSourceType.FILE, message="msg2", raw_message="msg2")

    store.add(event1, options=EventWriteOptions(event_id=str(event1.event_id)))
    with pytest.raises(EventIdCollisionError):
        store.add(event2, options=EventWriteOptions(event_id=str(event2.event_id)))
...
def test_max_events_eviction(make_log_event):
    ...
    store = InMemoryEventStore(EventStoreOptions(max_events=5, eviction_policy="oldest_inserted"))
...
def test_retention_policy(make_log_event):
    ...
    store = InMemoryEventStore(EventStoreOptions(retention_seconds=60, retention_basis="event_timestamp"))
...
def test_reject_new_policy(make_log_event):
    ...
    store = InMemoryEventStore(EventStoreOptions(max_events=1, eviction_policy="reject_new"))
