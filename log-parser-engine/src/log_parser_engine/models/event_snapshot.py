...
class EventStoreSnapshot(BaseModel):
    ...
    created_at: datetime = Field(description="The UTC timestamp when the snapshot was created.")
    
    event_count: int = Field(description="The number of events in the snapshot.")
    events: tuple[StoredEvent, ...] = Field(description="The sequence of stored events.")
    
    content_hash: str = Field(description="A SHA-256 hash of the snapshot's content for integrity verification.")
    estimated_size_bytes: int = Field(description="The total estimated size of all events in the snapshot.")
