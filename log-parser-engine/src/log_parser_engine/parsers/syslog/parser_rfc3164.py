...
from log_parser_engine.exceptions import NormalizationError, SyslogParserError
...
class Rfc3164SyslogParser(BaseParser):
    """Parse RFC 3164 syslog records into canonical LogEvent models."""
    ...
    def metadata(self) -> ParserMetadata:
        return ParserMetadata(
            name="rfc3164_syslog",
            display_name="RFC 3164 Syslog Parser",
            version="1.0.0",
            source_type=LogSourceType.SYSLOG,
            description="Parse RFC 3164 syslog records",
            ...
            thread_safe=True,
            experimental=False,
            tags=("syslog", "rfc3164", "classic", "bsd"),
        )
    ...
    def detect(
        ...
        text = raw_log.strip()
        if not text.startswith("<") or ">1 " in text[:12]:
            return DetectionResult.no_match(self.name, reason="not RFC 3164")
        try:
        ...
        except SyslogParserError:
            return DetectionResult.no_match(
                self.name,
                reason="invalid RFC 3164 header",
            )
        if record.version is not None:
            return DetectionResult.no_match(
                self.name,
                reason="RFC 5424 message detected",
            )
        confidence = 0.82
        signals = ["rfc3164"]
        if record.host:
        ...
        return DetectionResult.match(
            self.name,
            min(confidence, 1.0),
            reason="RFC 3164 syslog detected",
